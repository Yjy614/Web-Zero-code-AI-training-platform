...
与桌面端（PyQt6）并行的 Web 产品。支持 **目标检测** 与 **实例分割** 完整训练向导；默认开启 **演示模式**（Mock 训练，不自动下载权重）。关闭演示模式后可走本机 Ultralytics 真实训练。

## 目录

- `frontend/` — Vue 3 + TypeScript + Element Plus（向导、标注、资源页、推理试用）
- `backend/` — FastAPI + SQLite（任务队列、数据集、模型库、权重、推理）
- `backend/config/` — `web_config.yaml` 等（运行时设置见下方「配置」）
- `deploy/` — 部署示例
- `Web端项目说明书.md` — 产品与编码规格（含当前实现状态）
...
- Python 3.10+
- 真实训练 / 推理另需 `ultralytics`（及相应 torch，体积较大）
...
- **M1**：登录 / 侧栏 / 向导壳 / 演示横幅
- **M2**：数据集上传、清洗、YOLO 标注（`storage/datasets/<tt>/<用户名>/<名>/`）
- **M3**：配置 + Mock 训练 / 评估 / 导出
...
2. 管理员在「权重仓库」上传真实 `.pt`（如 `yolo11n.pt` / `yolov8n-seg.pt`）到服务器 `pretrained/<任务类型>/`，**不会自动从公网下载**。
3. 设置中关闭「演示模式」。
...
额外依赖：`Pillow`（图片清洗 / 推理出图）、`ultralytics`（真实训练 / 预标注 / 推理）、`echarts`（训练曲线，前端依赖）。

## 说明

- **不做**：OCR、浏览器自动下权重、公网 SaaS、打包 Windows 推理 EXE。
- **后期预留**：`ClusterJobRunner` 集群调度。
