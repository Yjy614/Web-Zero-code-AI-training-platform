...
> 编码时以本文为准；与桌面行为冲突时，以「Web 一期范围」章节为准。

---

## 0. 给 AI 的硬约束（先读）

1. **一期只做目标检测（detect）**；实例分割（segment）、OCR **不做 UI、不写业务接口**（可留目录/注释占位）。
2. **与桌面检测流程功能对齐**（7 步向导 + 资源管理 + 设置中与检测相关的部分）。
3. **现阶段是给领导看的产品壳**：默认开启「演示模式」——**禁止自动下载任何 `.pt` 权重**；训练/评估可走 **Mock 执行器** 跑通全流程页面。
...
8. 不要实现桌面端的「打包推理 EXE」到 Web；导出改为服务器上的文件下载（PT/ONNX）。
...
| 一期能力 | **仅检测**，流程与桌面检测对齐，便于领导演示 |
| 中期架构 | 后端部署在内网服务器；数据集落在集群可挂载的共享存储；训练任务下发到 GPU 集群 |
| 非目标（一期） | 分割、SAM、OCR、权重在线下载、公网 SaaS、本机 GPU 直连浏览器 |
...
- `segment` 任务类型、分割向导、SAM 服务。
- `ClusterJobRunner`：对接工厂 GPU 集群（Slurm / K8s / 自研调度均可）。
- 真实权重管理（管理员上传 `.pt` 到共享盘，禁止公网下载）。
...
| M5（后期） | ClusterJobRunner + 真实权重上传 + 真实训练 | 集群出 `best.pt` |
...
|----|------|----------|
| 登录 | 无 | 必须 |
| 任务类型 | detect + segment | 仅 detect |
| 训练位置 | 本机 GPU | Mock → 将来集群 |
| 数据位置 | 本机 `data/` | 服务器共享盘 |
